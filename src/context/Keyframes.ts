...
type DiffType = 'COUNTDOWN_DAYS' | 'COUNTDOWN_YEARS' | 'ANNIVERSARY' | 'PASSED_DAYS';
...
function generateFeatureKeyframesByDate(date: Dayjs) {
  ...
  if (now > date) {
    ...
    keyDateList.push(new Keyframe(date.add(diffYear, 'year'), 'ANNIVERSARY', diffYear));
    const diffDay = now.diff(date, 'day');
    if (diffDay <= 1000) {
      const next100 = (Math.floor(diffDay / 100) + 1) * 100;
      keyDateList.push(new Keyframe(date.add(next100, 'day'), 'PASSED_DAYS', next100));
      if (next100 < 1000) {
        keyDateList.push(new Keyframe(date.add(1000, 'day'), 'PASSED_DAYS', 1000));
      }
    } else {
      const next1000 = (Math.floor(diffDay / 1000) + 1) * 1000;
      keyDateList.push(new Keyframe(date.add(next1000, 'day'), 'PASSED_DAYS', next1000));
    }
  } else {
    // 未来时间，生成最近的100天、整年的列表
    const maxDiffYear = date.diff(now, 'year');
    for (let i = 1; i <= maxDiffYear; i++) {
      keyDateList.push(new Keyframe(date.subtract(i, 'year'), 'COUNTDOWN_YEARS', i));
    }
    const maxDiffDays = date.diff(now, 'day');
    if (maxDiffDays <= 10) {
      for (let i = 1; i <= maxDiffDays; i++) {
        keyDateList.push(new Keyframe(date.subtract(i, 'day'), 'COUNTDOWN_DAYS', i));
      }
    } else if (maxDiffDays <= 100) {
      const maxDiff10Days = Math.floor(maxDiffDays / 10);
      for (let i = 1; i <= maxDiff10Days; i++) {
        keyDateList.push(new Keyframe(date.subtract(i * 10, 'day'), 'COUNTDOWN_DAYS', i * 10));
      }
    } else {
      const maxDiff100Days = Math.floor(maxDiffDays / 100);
      for (let i = 1; i <= maxDiff100Days; i++) {
        keyDateList.push(new Keyframe(date.subtract(i * 100, 'day'), 'COUNTDOWN_DAYS', i * 100));
      }
    }
  }
